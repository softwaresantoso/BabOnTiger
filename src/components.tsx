...
function getRoleHomeLabel(role: string) {
  if (role === "admin") return "Admin";
  if (role === "barber") return "Workspace";
  return "Dashboard";
}

/**
 * Layout untuk halaman publik.
 * Menu akun sekarang mengikuti role user yang sedang login.
 */
export function PublicLayout() {
  const { profile, logout } = useAuth();

  const homePath = profile ? getRoleHome(profile.role) : "/login";
  const homeLabel = profile ? getRoleHomeLabel(profile.role) : "Masuk";
  ...
  return (
    <div className="app-shell">
      ...
      <footer className="footer">
        ...
        <span>Booking Barber Shop • PWA</span>
      </footer>
    </div>
  );
}
 ...
 * Jika user masuk ke route yang bukan miliknya, arahkan ke workspace role-nya,
  ...
  return (
    <div className="admin-shell">
      <aside className="sidebar">
        ...
        <nav>
          <NavLink to="/barber" end>
            <LayoutDashboard size={18} /> Hari Ini
          </NavLink>
        </nav>
        <div className="sidebar-bottom">
          <div className="muted">{profile?.name}</div>
