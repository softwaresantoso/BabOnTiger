...
export default function OwnerProducts() {
  ...
  const [name,setName]=useState(""); const [sku,setSku]=useState(""); const [cost,setCost]=useState(0); const [price,setPrice]=useState(0); const [stock,setStock]=useState(0); const [minStock,setMinStock]=useState(0); const [busy,setBusy]=useState<string|null>(null);
  ...
  async function add(){ if(!selectedBranchId || !name.trim()) return; try{await saveProduct({businessId:"",branchId:selectedBranchId,name:name.trim(),sku:sku.trim()||undefined,costPrice:Number(cost),sellingPrice:Number(price),stock:Number(stock),minimumStock:Number(minStock),active:true}); setName("");setSku("");setCost(0);setPrice(0);setStock(0);setMinStock(0);await load()}catch(e){setError(e instanceof Error?e.message:"Gagal menyimpan produk.")}}
